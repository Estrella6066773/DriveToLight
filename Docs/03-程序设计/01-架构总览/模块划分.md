...
| Input | 输入抽象、选中与指令；**首版以鼠标点击为主**（见 [平台与操作](../../02-系统设计/01-核心体验/平台与操作.md)）；MVP 暂并入 UI 运行时 | `InputController`（MVP） | `Yanxu.UI` |
...
首版已在 `Assets/Scripts/` 落地以下程序集与入口脚本：

| 模块 | 程序集 | 已落地入口 |
|------|--------|------------|
| Core | `Yanxu.Core` | `Abstractions/IGridQuery.cs`、`Abstractions/ITurnScheduler.cs`、`Abstractions/IWorkProgressStore.cs` |
| Map | `Yanxu.Map` | `HexCoord.cs`、`HexGridStore.cs`、`HexPathService.cs`、`OccupancyService.cs` |
| Turn | `Yanxu.Turn` | `TurnPhaseMachine.cs`、`ActionTableService.cs`、`Response/ResponseCheckPipeline.cs` |
| Team | `Yanxu.Team` | `Commands/TeamCommandQueueService.cs`、`Work/WorkProgressService.cs`、`TeamActionExecutionService.cs` |
| City | `Yanxu.City` | `MobileCityStateService.cs`、`SailingBoundaryGuard.cs` |
| UI | `Yanxu.UI` | `PrototypeSceneBootstrap.cs`、`TurnAndCommandPanel.cs`、`HexGridView.cs` |
...
| 2026-06-27 | 0.0.7 | 追加 MVP 实现对照：已落地程序集、入口脚本与命名空间前缀 |
