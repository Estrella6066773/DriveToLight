...
| 校验状态 | 待校验 |
| 日期 | 2026-06-23 |
| 关联设计 | [核心幻想](../../02-系统设计/01-核心体验/核心幻想.md)、[平台与操作](../../02-系统设计/01-核心体验/平台与操作.md)、[地图与移动](../../02-系统设计/02-地图与世界/地图与移动.md)、[城市模块化](../../02-系统设计/03-图层与地点/建筑层/README.md)、[四种核心资源](../../02-系统设计/04-资源与人口/四种核心资源.md)、[队伍系统](../../02-系统设计/06-单位与交战/队伍系统.md)、[视野系统](../../02-系统设计/06-单位与交战/视野系统.md)、[回合与行动表](../../02-系统设计/07-玩法循环/回合与行动表.md) |

## 模块列表

| 模块 | 职责 | 入口脚本 | 主要命名空间 |
|------|------|----------|--------------|
| Core | 生命周期、事件总线、存档、Effect / Ability 运行时 | | `Yanxu.Core`、`Yanxu.Core.Effects` |
...
| Team | 侦察 / 勘探 / 运输 / 工程队、人员与指令 | | `Yanxu.Team` |
| Vision | 侦察塔视野能力、地图已知层写入 | | `Yanxu.Vision` |
| Environment | 光带引擎（太阳推移）、环境效果结算（天气/地面表层/城市轨迹）、跨图层修正管线、章节环境配置 ─ 详见 [环境系统 · 程序设计](./环境系统.md) | | `Yanxu.Environment` |
| Exploration | 资源点揭示、采集设施 | | `Yanxu.Exploration` |
| Relation | 领袖关系存储与演算、组织间接传导、人口损失双轨折算、委托与贸易、关系行动（脱离/效忠）── 详见 [关系系统 · 程序设计](./关系系统.md) | | `Yanxu.Relation` |
| Sun | 太阳照射区纵向移动与光带；**v(t)=v₀+a·t** 驱动格修正；照射区离图后 **sun_motion_enabled=false**（全图暗渊带），见 [地图与移动 · 太阳照射与停用](../../02-系统设计/02-地图与世界/地图与移动.md#太阳照射区与移动停用) | | `Yanxu.Sun` |
...
- **文档职责**：文档追踪由文档流程维护，统一记录在 [待细化追踪](../../00-规范/README.md#待细化追踪) 与各文档的「待确认事项」中。
...
| GAS-lite 能力与 Buff 管线 | `Yanxu.Core.Effects/*`、`Yanxu.Team.Ability/DefaultTeamAbilityBootstrap.cs`、`Yanxu.Team.Work/WorkEfficiencyResolver.cs` | [Effect 与能力解析](../02-运行时逻辑/Effect与能力解析.md)、[队伍与指令执行](../02-运行时逻辑/队伍与指令执行.md) |
...
- [x] 平台与视角：**PC**、**3D 俯视**、**点击为主**（OPEN-004 已关闭）；默认相机与俯仰边界见 OPEN-036（已关闭）；无城市内部视图见 OPEN-038（已关闭）；旋转吸附等见 sy-17、sy-18；渲染管线见 sf-01。
...
| 2026-07-15 | 0.0.8 | **代码更名**：Communication 模块 → Vision；`Yanxu.Communication` → `Yanxu.Vision` |
| 2026-06-22 | 0.0.3 | 新增 Turn、Vision 模块；对齐队伍/视野/回合设计；校验状态改为部分符合 |
...
| 2026-06-29 | 0.0.12 | 增加 Core.Effects 模块，GAS-lite 替换旧能力闸门与数值 provider |
